...
import br.fapema.morholt.android.wizardpager.wizard.model.MultipleFixedChoicePage;
...
public class MultipleChoiceFragment extends MyListFragment implements PageInterface{
    private static final String ARG_KEY = "key";

    private PageFragmentCallbacks mCallbacks;
    private String mKey;
    private List<String> mChoices;
    private static MultipleFixedChoicePage mPage;
    private View rootView;
    
    public static MultipleChoiceFragment create(String pageId) {
    	Bundle args = new Bundle();
		args.putSerializable(PAGE_ID, pageId);

		MultipleChoiceFragment textFragment = new MultipleChoiceFragment();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mPage = (MultipleFixedChoicePage) getPage(pageId);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		((TextView) rootView.findViewById(R.id.comment)).setText(mPage.getComment());
	...
	public void update() {
        ...
        new Handler().post(new Runnable() {
            @Override
            public void run() {
                HashMap<String, Boolean> map = mPage.getValues();
                int i=0;
                for (String key : map.keySet()) {
                	listView.setItemChecked(i, map.get(key));
					i++;
				}
            }
        });
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        HashMap<String, Boolean> hashMapColumnValue = new HashMap<String, Boolean>();
        
        for (int i = 0; i < checkedPositions.size(); i++) {
        	hashMapColumnValue.put(mChoices.get(i), checkedPositions.valueAt(i));
        }

        mPage.saveValues(hashMapColumnValue);
